 ...
 */
	public function testLoginAfterIdentify() {
		...
		$event = new CakeEvent('Auth.afterIdentify', $this->Auth, array('Post' => $user));
 ...
 */
	public function testIsAuthorizedMissingFile() {
		...
		$this->Controller->Auth->isAuthorized(array('Post' => array('id' => 1)));
	}
 ...
 */
	public function testIsAuthorizedDelegation() {
		...
		$this->assertTrue($this->Auth->isAuthorized(array('Post'), $request));
	}
 ...
 */
	public function testUser() {
		...
		$result = $this->Auth->user();
		$this->assertEquals($data['Post'], $result);

		$result = $this->Auth->user('username');
		$this->assertEquals($data['Post']['username'], $result);

		$result = $this->Auth->user('Group.name');
		$this->assertEquals($data['Post']['Group']['name'], $result);
