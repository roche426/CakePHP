 ...
 */
	public function testRenderWithoutView() {
		...
		$data = array('users' => array('Post' => array('user1', 'user2')));
 ...
 */
	public function testRenderWithoutViewMultiple() {
		...
		$Response = new CakeResponse();
		$Controller = new Controller($Request, $Response);
		$data = array('no' => 'nope', 'Post' => 'fake', 'list' => array('item1', 'item2'));
		$Controller->set($data);
		$Controller->set('_serialize', array('no', 'Post'));
		$View = new XmlView($Controller);
		$this->assertSame('application/xml', $Response->type());
		$output = $View->render(false);
		$expected = array(
			'response' => array('no' => $data['no'], 'Post' => $data['Post'])
		);
		...
		$output = $View->render(false);
		$expected = array(
			'custom_name' => array('no' => $data['no'], 'Post' => $data['Post'])
		);
		$this->assertSame(Xml::build($expected)->asXML(), $output);
	}
 ...
 */
	public function testRenderWithoutViewMultipleAndAlias() {
		...
		$Response = new CakeResponse();
		$Controller = new Controller($Request, $Response);
		$data = array('original_name' => 'my epic name', 'Post' => 'fake', 'list' => array('item1', 'item2'));
		$Controller->set($data);
		$Controller->set('_serialize', array('new_name' => 'original_name', 'Post'));
		$View = new XmlView($Controller);
		$this->assertSame('application/xml', $Response->type());
		$output = $View->render(false);
		$expected = array(
			'response' => array('new_name' => $data['original_name'], 'Post' => $data['Post'])
		);
		$this->assertSame(Xml::build($expected)->asXML(), $output);
		...
		$output = $View->render(false);
		$expected = array(
			'custom_name' => array('new_name' => $data['original_name'], 'Post' => $data['Post'])
		);
		$this->assertSame(Xml::build($expected)->asXML(), $output);
	}
 ...
 */
	public function testRenderWithView() {
		...
		$expected = array(
			'users' => array('Post' => array('user1', 'user2'))
		);
