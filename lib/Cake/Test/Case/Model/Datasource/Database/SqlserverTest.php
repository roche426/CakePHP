 ...
 */
	public function testLimitOffsetHack() {
		$this->loadFixtures('Author', 'Post', 'Post');
		$query = array(
			'limit' => 2,
			'page' => 1,
			'order' => 'User.user ASC',
		...
		$this->assertEquals('garrett', $results[0]['Post']['Post']);
		$this->assertEquals('larry', $results[1]['Post']['Post']);
		...
		$this->assertEquals('mariano', $results[0]['Post']['Post']);
		$this->assertEquals('nate', $results[1]['Post']['Post']);
	}
