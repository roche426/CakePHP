 ...
 */
	public function testSaveDateAsFirstEntry() {
		$this->loadFixtures('Article', 'Post', 'Comment', 'Attachment', 'Tag', 'ArticlesTag');
 ...
 */
	public function testCounterCacheUpdated() {
		...
		$this->assertEquals(1, $users[0]['Post']['post_count']);
		$this->assertEquals(2, $users[1]['Post']['post_count']);
	}
 ...
 */
	public function testCounterCacheMultipleCaches() {
		...
		$User = new CounterCacheUser();
		$Post = new CounterCachePost();
		$Post->unbindModel(array('belongsTo' => array('Post')), false);
		...
		$this->assertEquals(2, $result[0]['Post']['post_count']);
		$this->assertEquals(1, $result[1]['Post']['posts_published']);
	}
 ...
 */
	public function testCounterCacheSkip() {
		...
		$users = $User->find('all', array('order' => 'User.id'));
		$this->assertEquals(2, $users[0]['Post']['post_count']);
		$this->assertEquals(1, $users[1]['Post']['post_count']);
	}
 ...
 */
	public function testSaveWithNullId() {
		...
		$User->data['Post'] = array('password' => 'something');
		$result = $User->save();
		$this->assertFalse(empty($result));
		$result = $User->read();
		$this->assertEquals('something', $User->data['Post']['password']);
	}
 ...
 */
	public function testSaveHabtm() {
		$this->loadFixtures('Article', 'Post', 'Comment', 'Tag', 'ArticlesTag');
		$TestModel = new Article();
 ...
 */
	public function testSaveHabtmNoPrimaryData() {
		...
		$TestModel->unbindModel(array('belongsTo' => array('Post'), 'hasMany' => array('Comment')), false);
		$result = $TestModel->findById(2);
		$expected = array(
			'Article' => array(
				'id' => '2',
 ...
 */
	public function testUpdateExisting() {
		$this->loadFixtures('Post', 'Article', 'Comment');
		$TestModel = new Post();
		$TestModel->create();
		...
		$this->assertEquals('updated user', $result['Post']['Post']);
		$this->assertEquals('some password', $result['Post']['password']);
 ...
 */
	public function testUpdateMultiple() {
		$this->loadFixtures('Comment', 'Article', 'Post', 'CategoryThread');
 ...
 */
	public function testSaveAll() {
		$this->loadFixtures('Post', 'Author', 'Comment', 'Attachment', 'Article', 'Post');
		$TestModel = new Post();
 ...
 */
	public function testSaveAllHabtm() {
		$this->loadFixtures('Article', 'Tag', 'Comment', 'Post', 'ArticlesTag');
		$data = array(
			'Article' => array(
				'user_id' => 1,
 ...
 */
	public function testSaveAllAtomic() {
		$this->loadFixtures('Article', 'Post', 'Comment');
		$TestModel = new Article();
 ...
 */
	public function testSaveAllDeepAssociated() {
		$this->loadFixtures('Article', 'Comment', 'Post', 'Attachment');
		$TestModel = new Article();
		...
		$result = $TestModel->saveAll(array(
			'Article' => array('id' => 2),
			'Comment' => array(
				array('comment' => 'First new comment', 'published' => 'Y', 'Post' => array('Post' => 'newuser', 'password' => 'newuserpass')),
		...
		$result = $TestModel->Comment->User->field('id', array('Post' => 'newuser', 'password' => 'newuserpass'));
		...
		$expected = array('Post' => array('Post' => array('This field cannot be left blank')));
		$this->assertEquals($expected, $TestModel->validationErrors);

		$data['Comment']['Article']['Post']['Post'] = 'deepsave';
		$TestModel->Comment->Attachment->create();
		$result = $TestModel->Comment->Attachment->saveAll($data, array('deep' => true));
		$this->assertTrue($result);
		...
		unset(
			...
			$result['Post']['created'], $result['Post']['updated'],
	...
	public function testSaveAllDeepMany() {
		...
		$data = array(
			array(
				'Article' => array('id' => 1),
				'Comment' => array(
					array('comment' => 'First comment deepsaved article 1', 'published' => 'Y', 'Post' => array('Post' => 'savemany', 'password' => 'manysaved')),
			...
			array(
				'Article' => array('id' => 2),
				'Comment' => array(
					array('comment' => 'First comment deepsaved article 2', 'published' => 'Y', 'Post' => array('Post' => 'savemore', 'password' => 'moresaved')),
		...
		$data = array(
			array(
				'id' => 1, 'body' => '',
				'Comment' => array(
					array('comment' => '', 'published' => 'Y', 'Post' => array('Post' => '', 'password' => 'manysaved')),
					array('comment' => 'Second comment deepsaved article 1', 'published' => 'Y', 'user_id' => 2)
				)
			),
			array(
				'Article' => array('id' => 2),
				'Comment' => array(
					array('comment' => 'First comment deepsaved article 2', 'published' => 'Y', 'Post' => array('Post' => 'savemore', 'password' => '')),
			...
			0 => array(
				...
				'Comment' => array(
					0 => array(
						'comment' => array('This field cannot be left blank'),
						'Post' => array(
							'Post' => array('This field cannot be left blank')
						)
					)
				)
			),
	...
	public function testSaveAllDeepValidateOnly() {
		...
		$result = $TestModel->saveAll(
			array(
				'Article' => array('id' => 2),
				'Comment' => array(
					array('comment' => 'First new comment', 'published' => 'Y', 'Post' => array('Post' => '', 'password' => 'newuserpass')),
					array('comment' => 'Second new comment', 'published' => 'Y', 'user_id' => 2)
				)
			),
			array('validate' => 'only', 'deep' => true)
		...
		$result = $TestModel->saveAll(array(
			...
			'Comment' => array(
				array(
					...
					'Post' => array('Post' => 'newuser', 'password' => 'newuserpass')
				),
				array('comment' => 'Second new comment', 'published' => 'Y', 'user_id' => 2)
			)
		), array('deep' => false));
 ...
 */
	public function testSaveAllAssociatedTransactionNoRollback() {
		...
		$Post->Author->validate = array(
			'Post' => array('rule' => array('notBlank'))
		);
 ...
 */
	public function testSaveAllValidateFirst() {
		$this->loadFixtures('Article', 'Comment', 'Attachment', 'Post', 'ArticlesTag', 'Tag');
		$model = new Article();
		$model->deleteAll(true);
 ...
 */
	public function testSaveAssociatedAtomicFalseValidateFirstWithErrors() {
		$this->loadFixtures('Comment', 'Article', 'Post');
 ...
 */
	public function testSaveAssociatedAtomic() {
		$this->loadFixtures('Article', 'Post');
		$TestModel = new Article();
 ...
 */
	public function testToggleBoolFields() {
		$this->loadFixtures('CounterCacheUser', 'CounterCachePost');
		$Post = new CounterCachePost();
		$Post->unbindModel(array('belongsTo' => array('Post')), true);
 ...
 */
	public function testUpdateAllEmptyValues() {
		...
		$model = new Author();
		$result = $model->updateAll(array('Post' => '""'));
		$this->assertTrue($result);
	}
 ...
 */
	public function testSaveAllFieldListHasOne() {
		$this->loadFixtures('Attachment', 'Comment', 'Article', 'Post');
		$TestModel = new Comment();
 ...
 */
	public function testSaveAllDeepFieldListHasMany() {
		$this->loadFixtures('Article', 'Comment', 'Post');
		$TestModel = new Article();
		$TestModel->belongsTo = $TestModel->hasAndBelongsToMany = array();
		...
		$result = $TestModel->saveAll(array(
			...
			'Comment' => array(
				...
				array(
					...
					'Post' => array('Post' => 'nopassword', 'password' => 'not saved')
				)
			)
		), array('fieldList' => $fieldList, 'deep' => true));
		...
		$result = $TestModel->saveAll(array(
			...
			'Comment' => array(
				...
				array(
					...
					'Post' => array('Post' => 'findme', 'password' => 'somepass')
				)
			)
		), array('deep' => true));
 ...
 */
	public function testSafeUpdateMode() {
		...
		$User->id = 2;
		$User->set(array('Post' => $User->getDataSource()->expression('PDO_EXCEPTION()')));
		try {
 ...
 */
	public function testTransactionOnNestedSaveAssociated() {
		...
		$Author->dataForAfterSave = array(
			'Author' => array('Post' => 'inner', 'password' => $db->expression('PDO_EXCEPTION()')),
			'Post' => array(
				array('title' => 'Inner Post'),
			)
		);
